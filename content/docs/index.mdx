...
- [Deployment](/operate/deployment): local Docker Compose, ClickHouse Cloud, production container model, environment variables, and migration workflow.
- [Railway deployment](/operate/railway): Railway container setup with ClickHouse or ClickHouse Cloud.
- [Helm deployment](/operate/helm): Kubernetes chart, autoscaling defaults, and install examples.
- [Verification](/operate/verification): what `npm run verify` and `npm run verify:e2e` prove.
...
- [SDK stability review](/project/sdk-stability): production reliability notes from comparing against mature browser analytics SDK patterns.
