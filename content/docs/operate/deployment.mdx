...
For platform-specific deployment examples, see [Railway deployment](/operate/railway) and [Helm deployment](/operate/helm).
...
The docs are a static Fumadocs/Next app in `apps/docs` with curated content under `content/docs`. GitHub Pages builds the app with `output: 'export'` and deploys the static `apps/docs/out` tree through `.github/workflows/pages.yml`.

In the repository settings, configure:

- **Pages > Build and deployment > Source**: GitHub Actions.

Pushes to `main` that change docs app files, `content/docs/**`, `openapi/**`, generated reference output, the lockfile, or the Pages workflow rebuild and deploy the docs site.
