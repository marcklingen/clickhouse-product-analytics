...
There is no project ID and no tenant column. `api_key` is stored on events as the accepted publishable credential, but it is not a tenant boundary. Do not write queries that assume `project_id`, `team_id`, or multi-tenancy.
...
| `api_key` | `LowCardinality(String)` | Accepted publishable API key. |
...
| Column | Meaning |
| --- | --- |
| `api_key` | Publishable API key on the events in the session. |
